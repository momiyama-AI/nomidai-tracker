...
ホーム画面の「のみだぬき」は、たまごっちやどうぶつの森のように、今月の支出状況に応じて生活感が変わる主役要素として扱う。

実装方式:

- v1.0 は Lottie / Rive / SpriteKit を追加せず、SwiftUI 標準アニメーションで実装する。
- 理由: 外部ライブラリゼロ方針を維持でき、`PhaseAnimator` / `KeyframeAnimator` / `withAnimation` / `task` により瞬き、呼吸、タップ反応、状態遷移を表現できるため。
- 後から PNG / アニメーション素材へ差し替えやすいよう、`CharacterView` は状態別 `assetName` を維持する。

常時アニメーション:

- 呼吸するようなゆるい拡大縮小。
- 数秒ごとの瞬き。
- たまに小さく傾く、上下に動くなどの待機モーション。
- タップ時は跳ねる、目を変える、光るなどのリアクションを行う。

生活レベル判定:

- 既存の `WealthLevel` 6段階を内部状態として継続利用する。
- 月次予算が設定済みの場合は、予算の日割り消化ペースを主軸にする。
- 月次予算が未設定の場合は、既存の基準値5,500円の日割り比較を主軸にする。
- `dryDayCount` が多い場合は少しリッチ側へ補正する。
- `pureAlcoholTenthsGram` は画面上で善悪評価せず、生活感の軽い補正としてのみ使う。
- 補正後の比率を既存の `WealthLevel` 境界に通して見た目を決める。

表示:

- 状態に応じて部屋、服、小物、吹き出しを変える。
- `grandRich` / `rich` は豪華な部屋、良い服、ごちそう寄り。
- `comfortable` / `normal` は小綺麗または普段の部屋。
- `broke` / `extremePoor` は質素な部屋、裸電球、段ボール、もやし寄り。
- 体型、姿勢、表情、衣服の乱れ、部屋の散らかり具合も状態差として表現する。
- 支出状況が良いほど姿勢が伸び、体型は丸くかわいく、衣服と部屋は整って見える。
- 支出状況が悪いほど姿勢がしょんぼりし、体型はやせ気味または崩れ気味、衣服と部屋は乱れて見える。
- ホームではキャラクターと部屋のアニメーションを前面に押し出し、単なる挿絵ではなく主役のインタラクティブ領域として扱う。
- キャラクターは状態ごとに呼吸速度、揺れ幅、タップ反応、背景小物の動きが変わる。
- セリフは支出ジョークに限定し、健康改善、節酒、禁酒、医療効果に見える文言は禁止する。
